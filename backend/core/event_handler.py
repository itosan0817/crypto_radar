...
async def handle_call_scheduled(w3: AsyncWeb3, log: dict):
    ...
    safe_print("\n" + "="*50)
    safe_print(f"🚨 [T=0] タイムロック変更予約(CallScheduled) を検知！")
    safe_print(f"🔗 Contract: {contract_addr}")
    safe_print(f"🧾 TX: https://basescan.org/tx/{tx_hash}")
    
    # Phase 2: データ構造解析
    decoded_data = decode_calldata(raw_data)
    safe_print(f"🧬 デコード済みデータ: {decoded_data}")
    
    # Phase 3: ラグゼロ価格取得 & スリッページ計算
    t0_price, slippage = await get_t0_price_and_slippage(w3, contract_addr)
    safe_print(f"📊 リアルタイム価格取得完了: ${t0_price} (想定スリッページ: {slippage}%)")
    ...
    safe_print(f"🤖 AI評価(一次): {ai_rank}級 (Score: {ai_score}) - {ai_summary}")
    
    # ランクが低すぎる場合は無視
    if ai_score < 60:
        safe_print("⚠️ スコアが基準未満のため、Discord通知と重い処理をスキップします。")
        return
...
async def handle_call_executed(w3: AsyncWeb3, log: dict):
    ...
    safe_print("\n" + "="*50)
    safe_print(f"🏁 [T+48] タイムロック実行(CallExecuted) を検知！")
    safe_print(f"🔗 Contract: {contract_addr}")
    
    # T+48時の価格を取得
    t48_price = await get_current_price(w3, contract_addr)
    safe_print(f"📊 実行時の価格取得完了: ${t48_price}")
