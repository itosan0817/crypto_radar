...
async def test_discord():
    print("🚀 Discord通知の送信テストを開始します...")
    
    # 1. Aerodrome Radar Test (DiscordService) - エリア：タイムロックなど
    print("1️⃣ Aerodrome Radar のエラー通知（生存確認）を送信...")
    ...
    print("2️⃣ Bribe Sniper のエラー通知（生存確認）を送信...")
    await notify_error(
    ...
    print("3️⃣ Bribe Sniper の架空のS級エントリー通知を送信...")
    dummy_pos = Position(
    ...
    # 4. Firestore 書き込みテスト
    print("4️⃣ Firestore 'bribe_positions' へのテストデータ保存を開始...")
    success = FirestoreSniperService.save_entry(dummy_pos)
    if success:
        print("✅ Firestore への書き込みに成功しました！")
    else:
        print("❌ Firestore への書き込みに失敗しました。ログを確認してください。")

    print("\n🏁 全てのテスト工程が完了しました！")
    print("Discordの通知と、Google CloudコンソールのFirestore画面をリロードして確認してください。")
