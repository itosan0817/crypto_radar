...
- **時間足切替**: チャートは 1分/15分/1時間/4時間/日足/週足 を選択可能。キャッシュに無い・更新が止まっている足は Binance から直接補完します（DBには書き込まない）。売買マーカーは選択中の足の位置に丸めて表示されます。
...
5. **現行に明確に勝った候補のみ** `config.local.yaml` に自動適用し、Discord 日次チャンネルへ通知
