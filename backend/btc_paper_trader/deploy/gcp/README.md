...
- **Aerodrome Radar 切替**: 画面上部の「🛰️ Aerodrome Radar」ボタンで、同じダッシュボード内でタイムロック変更予約の検知・答え合わせ表示に切り替えられます（ランク/答え合わせ状況での絞り込み・ページネーション付き）。`backend/services/firebase_service.py` 経由でFirestoreの `simulations` コレクションを読むため、`backend/serviceAccountKey.json` が必要です（aerodrome_radar.py と共通）。未配置の場合はこのタブでエラーメッセージが表示されるだけで、他の機能には影響しません。

#### エントリーモード（entry_mode）

`config.yaml` の `entry_mode`（ダッシュボードの「エントリーモード」からも変更可）で、エントリー判断の方式を切り替えられます。

- **regression**（従来方式）: モデル+パターン合成のスコアがしきい値を超えたらエントリー。トレンド時はモデル判定、レンジ時はグリッド逆張り。What-if・Claude自動チューニングはこのモードのみ対応。
- **claude_native**（案3・現在の既定）: 回帰分析・パターンマッチングを一切使わず、確定した足ごとに Claude 自身が市況だけを見て `long` / `short` / `flat` と自信度(0-100%)を判断する。自信度が `claude_native.min_confidence`（既定70%、ダッシュボードから変更可）を超えた場合のみ、その方向へエントリーする。ポジションサイズ・TP/SL幅は `claude_native.confidence_tier` により自信度に応じて連動（`risk.confidence_tier` とは別スケール）。TP/SL到達判定・日次損失ガード・クールダウン・ニュースフィルターなどのリスク管理は regression モードと完全共通。

  - **通知**: 確定した足ごとに1通（1時間足なら実質1時間に1回）、Claudeの判断・エントリーの有無・理由をDiscordの毎時チャンネルへ送信します（`claude_native.notify: false` で無効化可）。案1のセカンドオピニオン通知（`advisor.*`）とは独立で、regression モードでのみ動作します（claude_native では二重になるため自動的に無効）。
  - **ログ**: 閾値未満で見送った判断も含め、毎回 `data/claude_native_signal.jsonl` に記録します。後から異なる閾値でどうなっていたかを検証できるようにするためです。
  - **重要な制約**: このモードは **What-if シミュレーションと Claude自動チューニングの対象外** です。過去の各バーでClaudeを呼び直して判断を再現するのはコスト・再現性の面で現実的でないため、事前のバックテストができません。実運用（フォワードテスト）で結果を見ながら判断してください。ダッシュボードにもこの旨の注意書きが表示されます。
